...
if existing_data is not None and is_up_to_date:
    ...
    current_tickers = get_sp500_current()
    ...
    monthly_output_path = os.path.join(output_dir, "sp500_monthly_data.csv")
...
print(
    f"\nTop 10 des tickers avec le plus haut score total sur la période la plus récente ({latest_period.strftime('%Y-%m')}) :"
)
...
print(f"\nTop 10 des tickers avec le plus haut score total sur toutes les périodes :")
...
if all_top_performers:
    ...
    top_performers_output_path = os.path.join(
        output_dir, "sp500_top_performers_all_periods.csv"
    )
