#!/usr/bin/env python3
"""
Script to retrieve and analyze S&P 500 tickers historical data.
"""
...
def get_sp500_current() -> List[str]:
    ...
    try:
        ...
        logger.info(f"Current S&P 500 composition: {len(tickers)} companies")
        return tickers
    except requests.RequestException as e:
        logger.error(f"Error fetching S&P 500 list: {e}", exc_info=True)
        raise
...
def check_existing_data() -> Tuple[Optional[pd.DataFrame], bool]:
    ...
    csv_path = os.path.join("data", "history", "sp500_monthly_data.csv")
...
def _save_top_performers(performance_df: pd.DataFrame, output_dir: str) -> None:
    ...
    if all_top_performers:
        ...
        output_path = os.path.join(output_dir, "sp500_top_performers_all_periods.csv")
...
def main() -> None:
    ...
    if existing_data is not None and is_up_to_date:
        ...
        monthly_data = get_monthly_data(get_sp500_current(), months=13)
        output_dir = os.path.join("data", "history")
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, "sp500_monthly_data.csv")
...
if __name__ == "__main__":
    logger.info("Starting S&P 500 tickers history retrieval...")
    main()
    logger.info("S&P 500 analysis completed")
